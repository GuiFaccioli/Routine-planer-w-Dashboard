...
export default async function DayPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  return <AppShell><div className="container py-10"><div className="mb-8 flex flex-wrap items-end justify-between gap-5"><div><p className="mb-2 text-sm font-bold uppercase tracking-[.18em] text-[var(--accent)]">meu dia</p><h1 className="text-4xl font-black tracking-tight">{new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "numeric", month: "long" }).format(date)}</h1><p className="mt-2 muted">{completed} de {tasks.length} tarefas concluídas · {Math.floor(totalMinutes / 60)}h{String(totalMinutes % 60).padStart(2, "0")} planejadas</p></div><div className="flex items-center gap-2"><Link className="btn btn-secondary" href={`/day?date=${shiftDate(date, -1)}`}>←</Link><Link className="btn btn-secondary" href="/day">Hoje</Link><Link className="btn btn-secondary" href={`/day?date=${shiftDate(date, 1)}`}>→</Link></div></div>{active && <ActiveTimer task={active.task} sessions={activeSessions} />}<DayTimeline tasks={tasks} activeTaskId={active?.task.id} /></div></AppShell>;
}

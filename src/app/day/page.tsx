...
export default async function DayPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  return <AppShell><div className="container page"><div className="hero-row"><div><p className="eyebrow">Meu dia</p><h1 className="page-title">{new Intl.DateTimeFormat("pt-BR", { weekday: "long", day: "numeric", month: "long" }).format(date)}</h1><p className="summary-line"><strong>{completed}</strong> de {tasks.length} tarefas concluídas · <strong>{Math.floor(totalMinutes / 60)}h{String(totalMinutes % 60).padStart(2, "0")}</strong> planejadas</p></div><div className="date-actions"><Link className="btn btn-outline" href={`/day?date=${shiftDate(date, -1)}`} aria-label="Dia anterior">←</Link><Link className="btn btn-secondary" href="/day">Hoje</Link><Link className="btn btn-outline" href={`/day?date=${shiftDate(date, 1)}`} aria-label="Próximo dia">→</Link></div></div>{active && <ActiveTimer task={active.task} sessions={activeSessions} />}<div className="section-heading mt-10"><h2>Agenda do dia</h2><span className="help-text">{tasks.length} itens</span></div><DayTimeline tasks={tasks} activeTaskId={active?.task.id} /></div></AppShell>;
}
